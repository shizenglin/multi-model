...
def build_batch( raw_data_path, batch_index, start_index, data_type ):
    ...
    #            read images
    #---------------------------------------
    data_file = gzip.open( file_name + 'dat.mat.gz','r')
    # skip header
    data_file.read(8)
    ...
    result_data = np.zeros( (NUM_IMAGE_BATCH,2*(IMAGE_TARGET_SIZE)**2), np.uint8, order='C' )
    # read image data
    ...
    #            read labels
    #---------------------------------------
    label_file = gzip.open( file_name + 'cat.mat.gz', 'r' )
    # skip header
    label_file.read(8);
...
def main():
    raw_data_path = '/home/shizenglin/masterwork/data/norb'
    image_mean_list = []
    # training data
    """for ii in range(1,3):
    #for ii in range(1,):
        build_batch( raw_data_path, ii, 0 , 'training' )
